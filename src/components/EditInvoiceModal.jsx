...
export default function EditInvoiceModal({ invoice, onClose, onSaved, onPrint, paperSize, onPaperSizeChange }) {
  ...
  async function handleSaveItems() {
    if (items.some((it) => !it.batch_id || !it.quantity || !it.unit_price)) {
      flash('error', 'يرجى ملء كل حقول البنود');
      return;
    }
    setSaving(true);
    try {
      ...
      flash('error', err.message || 'حدث خطأ');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleAddPayment() {
    ...
    if (!amount || amount <= 0) { flash('error', 'أدخل مبلغ صحيح'); return; }
  ...
  async function handleEditPayment(p) {
    const val = prompt('المبلغ الجديد:', p.amount);
      ...
      <div className="bg-surface-container-lowest rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col border border-outline-variant/40">
        ...
        <div className="flex gap-1 px-6 pt-4">
          {['items', 'payments'].map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              ...
              {tab === 'items' ? <span className="flex items-center gap-1.5"><Package size={14} />البنود</span> : <span className="flex items-center gap-1.5"><CreditCard size={14} />الدفعات</span>}
            </button>
          ))}
        </div>
            ...
            <>
                ...
                <div key={idx} className="grid grid-cols-12 gap-2 items-start p-3 rounded-xl bg-surface-container-low/40 border border-outline-variant/30">
                  <div className="col-span-5">
                    ...
                    {isPurchase && (() => {
                      ...
                      return sold > 0 ? (
                        <p className="text-[11px] text-amber-600 mt-1">تم بيع {sold.toFixed(2)} وحدة — الكمية الجديدة يجب أن تكون ≥ {sold.toFixed(2)}</p>
                      ) : null;
                    })()}
                  </div>
                  <div className="col-span-3">
                    <label className="block text-label-sm text-muted-steel mb-1 uppercase tracking-wider">الكمية</label>
                    <input
                  ...
                  <div className="col-span-3">
                    <label className="block text-label-sm text-muted-steel mb-1 uppercase tracking-wider">السعر</label>
                    <input
                  ...
                  <div className="col-span-12 text-right">
                    <span className="text-label-sm text-muted-steel">الإجمالي الفرعي: </span>
              ...
              {!isPurchase ? (
                ...
                <div className="text-center p-3 text-label-sm text-sky-700 bg-sky-50 rounded-xl border border-sky-200">
                  يمكنك تعديل الكميات والأسعار فقط. لإضافة أو حذف بنود، يرجى حذف الفاتورة وإعادة إنشائها.
                </div>
              )}
            </>
          )}
            ...
            <>
              <div className="flex gap-2 p-3 rounded-xl bg-surface-container-low/40 border border-outline-variant/30">
                <div className="flex-1">
                  <label className="block text-label-sm text-muted-steel mb-1 uppercase tracking-wider">دفعة جديدة</label>
                  <input type="number" step="0.01" min="0" placeholder="المبلغ..." value={newPaymentAmount} onChange={(e) => setNewPaymentAmount(e.target.value)} className={inputCls} />
                </div>
              ...
              {loadingPayments ? (
                ...
                <p className="text-center text-muted-steel text-body-sm py-6">لا توجد دفعات مسجلة</p>
              ) : (
                  ...
                  {payments.map((p) => (
                    <div key={p.id} className="flex items-center justify-between px-4 py-3 rounded-xl border border-outline-variant/30 bg-surface-container-lowest">
                      <div>
                        ...
                        <p className="text-label-sm text-muted-steel">{p.payment_date ? new Date(p.payment_date).toLocaleDateString('ar-EG') : '—'}</p>
                      </div>
                      <div className="flex gap-1">
                        <button onClick={() => handleEditPayment(p)} className="px-3 py-1.5 text-label-sm rounded-lg text-accent hover:bg-accent-surface transition-colors cursor-pointer btn-tactile">تعديل</button>
                        <button onClick={() => handleDeletePayment(p)} className="px-3 py-1.5 text-label-sm rounded-lg text-red-500 hover:bg-red-50 transition-colors cursor-pointer btn-tactile">حذف</button>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="px-6 py-4 border-t border-outline-variant/30 flex items-center justify-between gap-4 bg-surface-container-low/30 rounded-b-2xl">
          <div className="flex gap-4 text-label-sm">
            <span className="text-muted-steel">الإجمالي: <span className="font-mono-tabular text-charcoal-ink">EGP {total.toFixed(2)}</span></span>
            <span className="text-muted-steel">المدفوع: <span className="font-mono-tabular text-green-600">EGP {totalPaid.toFixed(2)}</span></span>
            <span className={`font-medium ${balance > 0 ? 'text-amber-600' : 'text-green-600'}`}>
              المتبقي: EGP {balance.toFixed(2)}
            </span>
          </div>
