...
export default function SettingsView() {
  ...
  async function handleSaveSettings() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(error.detail || t('settings.saveFailedMessage'));
      }
    ...
    } catch (err) { notifications.show({ title: t('settings.saveFailed'), message: err?.message || t('settings.saveFailedMessage'), color: 'red' }); }
  ...
  async function handleUploadLogo() {
    ...
    try {
      ...
      if (!res.ok) {
        const error = await res.json().catch(() => ({}));
        throw new Error(error.detail || t('settings.uploadFailedMessage'));
      }
      ...
      setShowSaved(true);
      setTimeout(() => setShowSaved(false), 2500);
    } catch (err) { notifications.show({ title: t('settings.uploadFailed'), message: err?.message || t('settings.uploadFailedMessage'), color: 'red' }); }
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 animate-fade-in-up">
        <div>
          ...
          <p className="text-body-base text-muted-steel mt-1">{t('settings.subtitle')}</p>
        </div>
        <button onClick={handleSaveSettings} disabled={saving}
          ...
          {showSaved ? t('settings.saved') : t('settings.saveSettings')}
        </button>
      </div>
        ...
        <div className="p-6 border-b border-outline-variant/30">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-body-sm text-muted-steel mt-1">{t('settings.brandIdentityHint')}</p>
            </div>
          </div>
        </div>
      ...
      <section className="bg-surface-container-lowest border border-outline-variant/60 rounded-2xl shadow-whisper animate-fade-in-up stagger-2">
        ...
        <div className="p-6">
          ...
          <textarea value={formData.default_invoice_footer} onChange={(e) => setFormData({ ...formData, default_invoice_footer: e.target.value })} placeholder={t('settings.footerPlaceholder')} rows={4}
