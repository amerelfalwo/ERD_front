...
function SupplierModal({ isOpen, onClose, supplier, onSaved }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      onSaved();
      onClose();
    } catch (err) { notifications.show({ title: t('common.error'), message: err?.response?.data?.detail || err?.message || t('suppliers.errorSaving'), color: 'red' }); }
...
function SupplierCard({ supplier, onEdit, onDelete }) {
  ...
  return (
    <Card shadow="xs" radius="lg" padding={0} className="border border-outline-variant/30 overflow-hidden flex flex-col hover:shadow-md transition-shadow bg-surface-container-lowest">
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="flex items-center gap-2 text-muted-steel mb-6 mt-2">
          <IconPhone size={14} />
          <span className="text-sm">{supplier.phone || t('common.noData')}</span>
        </div>

        <div className="mt-auto flex items-center justify-between pt-4 border-t border-outline-variant/30">
          <Flex gap="sm">
            <Tooltip label={t('common.edit')}>
              <ActionIcon variant="subtle" color="violet" onClick={() => onEdit(supplier)} radius="md">
                <IconPencil size={18} />
              </ActionIcon>
            </Tooltip>
            <Tooltip label={t('common.delete')}>
              <ActionIcon variant="subtle" color="red" onClick={() => onDelete(supplier)} radius="md">
                <IconTrash size={18} />
              </ActionIcon>
            </Tooltip>
          </Flex>
...
export default function SuppliersView() {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      fetchSuppliers();
    } catch (err) { notifications.show({ title: t('common.error'), message: err?.response?.data?.detail || err?.message || t('suppliers.errorDeleting'), color: 'red' }); }
  ...
  const totalPayables = suppliers.reduce((sum, s) => sum + (Number(s.calculated_balance) > 0 ? Number(s.calculated_balance) : 0), 0);
  const activeSuppliers = suppliers.filter(s => Number(s.calculated_balance || 0) !== 0).length;
    ...
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-fade-in-up">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <Select
            data={[
              { value: 'a-z', label: t('suppliers.sortNameAZ') },
              { value: 'z-a', label: t('suppliers.sortNameZA') },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-fade-in-up stagger-1">
        ...
        <div className="bg-surface-container-lowest p-5 rounded-2xl border border-outline-variant/50 shadow-sm flex flex-col">
          <p className="text-label-sm text-muted-steel uppercase tracking-wider mb-1">{t('suppliers.activeAccounts')}</p>
          <p className="text-3xl font-semibold text-charcoal-ink">{activeSuppliers}</p>
        </div>
      </div>
          ...
          <div className="bg-surface-container-lowest rounded-2xl shadow-2xl border border-outline-variant/60 w-full max-w-md overflow-hidden animate-fade-in-up">
            <div className="p-6">
              ...
              <p className="text-muted-steel text-sm leading-relaxed mb-6" dir="auto">
                {t('suppliers.confirmDeleteMessagePlain', { name: supplierToDelete.name })}
              </p>
